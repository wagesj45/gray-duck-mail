...
{
    /// <summary> Attribute for declaring a controller route method as accessable only internally. </summary>
    ...
    public class InternalAccessOnlyAttribute : ActionFilterAttribute
    {
        #region Members
        ...
        /// <exception cref="AccessViolationException"> Thrown when an attempt was made to access a sensative method from an external source. </exception>
        ...
        public override void OnActionExecuting(ActionExecutingContext filterContext)
        {
            ...
            {
                logger.Error("The request is being processed from the designated external port. Request Origin: {0}", filterContext.HttpContext.Connection.RemoteIpAddress);

                throw new AccessViolationException("The request is being processed from the designated external port.");
            }
            else
            {
                ...
                throw new AccessViolationException("The request is being processed from an unknown port.");
            }
        } 
